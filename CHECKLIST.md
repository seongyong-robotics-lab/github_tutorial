...
- [x] Notion에 `SYS-REQ` DB 생성 — `REQ ID`, `요구사항`, `승인 상태`, `구현 상태`, `GitHub Issue`, `검증 방법`
      (`ID` 는 Notion 예약어와 충돌해 API 로 값을 못 넣는다 → `REQ ID` 로)
- [x] 요구사항 8개를 `REQ-001` ~ `REQ-008`로 작성, 전부 실제 이슈와 매핑
- [x] 이슈 제목을 `[REQ-003] ...` 형식으로 — 2건 손으로, 6건 CLI 백필
- [x] Notion REQ 페이지에 이슈 URL + GitHub 검색 링크
- [x] 이슈를 닫을 때 Notion 상태도 손으로 변경 (REQ-001 어긋난 것 수정)
- [x] **자문: 어디가 제일 귀찮았나?** → **Notion 상태 바꾸기 + 제목에 REQ ID 붙이기**

> **압축 진행.** 2주 대신 한 세션에 체험했다. 대신 "무엇이 귀찮은가"는 실제로 손으로
> 해본 뒤에 답했고, 그 답이 자동화 순서를 정했다.
>
> **B(제목 규약)가 A(상태 동기화)의 전제조건**이다. 제목에 REQ ID 가 없으면 동기화
> 스크립트가 매칭할 근거 자체가 없다. 백필 전에는 12건 중 2건만 대상이었다.
>
> **자동화 ≠ 백필.** 자동화를 켜도 과거는 안 바뀐다. 기존 6건은 일회성 마이그레이션으로
> 처리했다. 새 시스템을 도입할 때 항상 나오는 구분이다.

### 6-2. 반자동

- [x] 이슈 템플릿의 REQ ID 칸 → **워크플로가 제목에 자동 삽입** (`req-id-title.yml`)
- [x] PR 템플릿에도 REQ ID 줄 추가 (Stage 2 에서 미리 넣어둠)
- [x] Notion REQ 페이지 8건에 GitHub 검색 링크 저장
- [ ] 주 1회 "동기화 점검" 루틴 — 제목이 바뀐 이슈는 자동화가 못 잡으므로 필요

> **검색 링크는 코드 0줄짜리 자동화다.** 제목 규약(`[REQ-003]`)이 서 있으면
> `github.com/search?q=repo:...+REQ-003` 이 항상 최신 목록을 준다. 갱신할 필요가 없고,
> 한 REQ 에 이슈가 여러 개여도 전부 잡힌다.
> **"자동화"를 코드로만 생각하면 이런 해법을 놓친다.**

### 6-3. 자동화

- [x] Notion 연결(내부) 생성 — 기능은 읽기/업데이트만. 삽입·댓글·사용자 정보 없음
- [x] **DB 에 연결 추가** ← 이걸 빼먹으면 404. 토큰 문제로 착각하기 쉬움
- [x] repo Secrets에 `NOTION_TOKEN`, `NOTION_DB_ID` 등록
- [x] `tools/sync_notion.py` 작성 + `decide_status` 단위 테스트 6건
- [x] `.github/workflows/sync-notion.yml` 추가
- [x] 실제 이슈(#5) 종료 → `[ok] REQ-003 -> 완료 (남은 열린 이슈 0건)` 확인
- [x] 💥 `[REQ-999]` 이슈(#32) 종료 → 워크플로 **failure**, 에러 메시지에 원인 명시
- [x] 💥 양쪽에서 동시에 다르게 변경 → **GitHub 이 덮어씀. 알림 없이 사라짐**
- [x] 그 결과로 `docs/notion-boundary.md` 5개 항목 추가

**✅ 완료 조건** — 이슈를 닫으면 Notion이 자동으로 바뀜 + 원본 규칙이 문서화됨 → **달성**

> **정답지와 다르게 만든 것 4가지** (전부 실제로 굴려보다 나온 것)
>
> | | `_answers/stage6/` | 이 repo |
> |---|---|---|
> | 속성 구성 | `상태` 하나 | **`승인 상태` / `구현 상태` 분리** |
> | 여러 이슈가 한 REQ 에 | 하나만 닫혀도 완료 | **열린 이슈 남으면 구현 중** |
> | `not_planned` 종료 | 완료 | **보류** |
> | 404 에러 | 그대로 전파 | **DB 연결 누락을 먼저 의심하라고 안내** |
>
> **트리거에 `opened` 를 추가한 것도 사후 수정이다.** 처음엔 `closed`/`reopened` 만 잡았는데,
> 완료된 REQ 에 새 이슈가 열리면 Notion 이 `완료` 로 남는 구멍이 있었다.
> 자동화는 상태 전이를 **양쪽 다** 덮어야 하는데, 만들 때는 눈앞의 시나리오만 보게 된다.
